...
export function parseCliArgs(argv) {
    ...
    if (!schema)
        return { command: "error", message: `Comando sconosciuto: ${comando}\n\n${USO}` };
    const valori = {};
    const flag = new Set();
    for (let i = 0; i < resto.length; i++) {
        ...
        if (schema.flag.includes(a)) {
            ...
            if (v === undefined || v.startsWith("--"))
                return { command: "error", message: `L'opzione ${a} richiede un valore.` };
            valori[a] = v;
            i++;
        }
        else {
    ...
    for (const o of schema.obbligatorie) {
        if (!valori[o])
            return { command: "error", message: `Manca l'opzione obbligatoria ${o}.\n\n${USO}` };
    }
