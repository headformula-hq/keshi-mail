...
import { emailKeshi, type EmailKeshiOpts } from "./template.js";
import { otpSupabaseTemplate } from "./otp.js";
import { escapeHtml } from "./escape.js";
import type { KeshiMailBrand } from "./brand.js";

// I dieci template della tavola di design "Email Keshi" (6 settembre 2026),
// con dati di esempio: servono a vedere il layout nel browser, non a spedire.

export type TemplateEsempio = { slug: string; titolo: string; subject: string; opts: EmailKeshiOpts };

const APP = "https://app.keshilabs.com";

export const TEMPLATE_ESEMPIO: TemplateEsempio[] = [
  {
...
export function writePreview({ out, product }: { out: string; product?: string }): string[] {
  const brand: Partial<KeshiMailBrand> = product ? { product } : {};
  mkdirSync(out, { recursive: true });
  const voci: { file: string; titolo: string; subject: string }[] = [];
