...
export type EmailMessage = {
  ...
  html: string;
  from?: string;
  attachments?: { filename: string; content: Uint8Array | string }[];
};
...
export type SendEmailResult =
  | { ok: true; id?: string }
  | { ok: false; reason: "no_api_key" | "http" | "network"; status?: number };

// Mittente di default: "keshi" minuscolo, come il marchio.
export const DEFAULT_FROM = "keshi <noreply@keshilabs.com>";

export async function sendEmail(msg: EmailMessage, opts: SendEmailOpts = {}): Promise<SendEmailResult> {
  ...
  try {
    ...
    const id = await r
      .json()
      .then((j: unknown) => (j && typeof j === "object" && typeof (j as { id?: unknown }).id === "string" ? (j as { id: string }).id : undefined))
      .catch(() => undefined);
...
function base64(c: Uint8Array | string): string {
